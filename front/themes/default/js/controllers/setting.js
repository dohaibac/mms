app.controller('SettingCtrl', function($scope, $http, $SettingService) {
  ...
  $scope.disabled = function() {
    if (!$scope.setting.num_commands_per_day || !$scope.setting.num_days_pd_pending || !$scope.setting.num_days_pd_transfer || 
    ...
    return $scope.setting.num_commands_per_day.length > 0 && $scope.setting.num_days_pd_pending.length > 0 && $scope.setting.num_days_pd_transfer.length > 0 && 
